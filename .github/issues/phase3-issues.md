  ...
  const useTodoStore = create((set, get) => ({
    ...
    fetchTodos: async () => {
      ...
      try {
        ...
        set({
          error:
            error.response?.data?.error?.message || "할일 조회에 실패했습니다",
          isLoading: false,
        });
      }
    },
    ...
    createTodo: async (todoData) => {
      ...
      try {
        ...
        set({
          error:
            error.response?.data?.error?.message || "할일 생성에 실패했습니다",
          isLoading: false,
        });
      }
    },
      ...
      <header className="bg-white shadow-md">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <nav className="flex gap-4 items-center">
            ...
            <Link to="/trash" className="text-gray-700 hover:text-primary">
              휴지통
            </Link>
            <Link to="/holidays" className="text-gray-700 hover:text-primary">
              국경일
            </Link>
...
- **코드 예시**:
  ...
  const TodoListPage = () => {
    const { todos, isLoading, error, fetchTodos, completeTodo, deleteTodo } =
      useTodoStore();
...
- **코드 예시**:
  ...
  const TrashPage = () => {
    const { todos, fetchTodos, restoreTodo, permanentlyDelete, setFilters } =
      useTodoStore();
